...
export function Toast() {
    ...
    return (
        <Animated.View
            style={[styles.container, animatedStyle, { top: insets.top + 8, marginHorizontal: spacing.md }]}
        ...
        >
            ...
            >
                <View style={[styles.iconWrap, { backgroundColor: tint + '15', borderRadius: radius.full }]}>
                    <Ionicons name={ICON_MAP[current.type]} size={18} color={tint} />
                </View>
                <Text style={[typography.label, { color: colors.text, flex: 1, marginLeft: spacing.sm }]} numberOfLines={2}>
                    {current.message}
                </Text>
            </Pressable>
        </Animated.View>
