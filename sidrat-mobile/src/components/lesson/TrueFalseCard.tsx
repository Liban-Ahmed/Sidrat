...
export function TrueFalseCard({ block, onAnswer }: Props) {
    ...
    const getButtonStyle = (value: boolean) => {
        ...
        if (isCorrect) {
            return { bg: colors.successMuted, border: colors.success, text: colors.success, iconColor: colors.success, shadow: colors.success };
        }
        return { bg: colors.errorMuted, border: colors.error, text: colors.error, iconColor: colors.error, shadow: colors.error };
    };
    ...
    return (
        <View style={styles.container}>
            <Animated.View entering={FadeInDown.delay(100).duration(500)}>
                <FormattedText
                    style={[typography.title3, { color: colors.text, textAlign: 'center', marginBottom: 32, lineHeight: 32 }]}
                >
                    {block.statement}
                </FormattedText>
            </Animated.View>
            ...
            <View style={styles.buttons}>
                {([true, false] as const).map((value, i) => {
                    ...
                    return (
                        <Animated.View key={String(value)} entering={FadeInUp.delay(250 + i * 100).duration(400)} style={{ flex: 1 }}>
                            <Pressable
                            ...
                            >
                                <View style={[styles.iconBg, { backgroundColor: s.iconColor + '14', borderRadius: radius.full }]}>
                                    <Ionicons name={value ? 'checkmark-circle' : 'close-circle'} size={30} color={s.iconColor} />
                                </View>
