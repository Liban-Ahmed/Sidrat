...
export function TapWordCard({ block, onAnswer, accentColor }: Props) {
  ...
  const nextExpected =
    selected.length < block.correctSentence.length ? block.correctSentence[selected.length] : null;
            ...
            {selected.map((word, i) => (
              <Animated.View key={`${word}-${i}`} entering={ZoomIn.duration(200)} layout={Layout}>
                ...
                >
                  ...
                  {showResult && <Ionicons name="checkmark" size={14} color={colors.success} />}
                </View>
              </Animated.View>
            ))}
          </View>
              ...
              >
                <Text
                  style={[
                    typography.callout,
                    {
                      color: isWrong ? colors.error : isHinted ? brand.accent : colors.text,
                    },
                  ]}
                >
                  {word}
