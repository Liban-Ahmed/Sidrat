            ...
            <ScrollView contentContainerStyle={{ paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
                ...
                <Animated.View entering={FadeIn.duration(400)}>
                    <LinearGradient
                        ...
                        style={[styles.profileHeader, { paddingHorizontal: spacing.lg, paddingTop: spacing.xl + 54, paddingBottom: spacing.lg }]}
                    >
                        <View style={styles.profileRow}>
                            ...
                            <View style={{ marginLeft: spacing.md, flex: 1 }}>
                                ...
                                <Text style={[typography.bodySmall, { color: 'rgba(255,255,255,0.75)', marginTop: 2 }]}>
                                    {getAge(child.birthYear)} years old
                                </Text>
                                <Text style={[typography.caption, { color: 'rgba(255,255,255,0.6)', marginTop: 6, fontStyle: 'italic' }]}>
                                    {getMotivation(child)}
                                </Text>
                            </View>
                        </View>
                        <View style={[styles.levelSection, { marginTop: spacing.md }]}>
                            <View style={[styles.levelBadge, { backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: radius.full }]}>
                                <Ionicons name="star" size={12} color="#FFF" />
                                <Text style={[typography.captionBold, { color: '#FFF', marginLeft: 4 }]}>Level {level}</Text>
                            </View>
                            <Text style={[typography.caption, { color: 'rgba(255,255,255,0.8)' }]}>{xpInLevel}/100 XP</Text>
                        </View>
                        <View style={[styles.xpBar, { backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: radius.full, marginTop: spacing.xs }]}>
                            <View
                                style={[
                                    styles.xpFill,
                                    {
                ...
                <Animated.View entering={FadeInDown.delay(delay(0)).duration(400)} style={[styles.statsRow, { paddingHorizontal: spacing.lg, marginTop: spacing.lg }]}>
                    {STATS.map((stat) => (
                        <View key={stat.label} style={styles.statBadge}>
                            <View style={[styles.statCircle, { backgroundColor: stat.color + (isDark ? '25' : '15') }]}>
                                <Ionicons name={stat.icon} size={18} color={stat.color} />
                            </View>
                            <Text style={[typography.title3, { color: colors.text, marginTop: 4, fontSize: 15 }]}>{stat.value}</Text>
                            <Text style={[typography.caption, { color: colors.textTertiary, marginTop: 1, fontSize: 10 }]}>{stat.label}</Text>
                        </View>
                    ))}
                </Animated.View>

                {/* ── This Week: crescent streak + weekly goal in one card ── */}
                <Animated.View entering={FadeInDown.delay(delay(1)).duration(400)} style={{ paddingHorizontal: spacing.lg, marginTop: spacing.xxl }}>
                    ...
                    <Card variant="glass" style={{ marginTop: spacing.sm }}>
                                ...
                                <View key={i} style={styles.streakDayCol}>
                                    ...
                                    <Text
                                        style={[
                                            typography.caption,
                                            {
                                                color: day.active || day.isToday ? colors.textSecondary : colors.textTertiary,
                                                marginTop: 3,
                                                fontSize: 10,
                        ...
                        <View style={[styles.weekGoalRow]}>
                            <ProgressRing
                                ...
                                glow={weekProgress >= 1}
                            >
                                <Text style={[typography.captionBold, { color: weekProgress >= 1 ? brand.secondary : brand.primary, fontSize: 10 }]}>{completedCount}/5</Text>
                            </ProgressRing>
                            <View style={{ marginLeft: spacing.md, flex: 1 }}>
                                <Text style={[typography.label, { color: colors.text }]}>Weekly Goal</Text>
                                <View style={[styles.miniBar, { backgroundColor: colors.surfaceTertiary, borderRadius: radius.full, marginTop: 4 }]}>
                                    <View style={[styles.miniBarFill, { backgroundColor: weekProgress >= 1 ? brand.secondary : brand.primary, borderRadius: radius.full, width: `${weekProgress * 100}%` as `${number}%` }]} />
                                </View>
                                <Text style={[typography.caption, { color: colors.textSecondary, marginTop: 3 }]}>
                                    {weekProgress >= 1 ? "Masha'Allah! All done." : `${5 - completedCount} lessons left this week.`}
                                </Text>
                            </View>
                        </View>
                    </Card>
                ...
                {/* ── Overall Progress ── */}
                <Animated.View entering={FadeInDown.delay(delay(2)).duration(400)} style={{ paddingHorizontal: spacing.lg, marginTop: spacing.xxl }}>
                    <Text style={[typography.title3, { color: colors.text }]}>Overall Progress</Text>
                    <Card variant="glass" style={{ marginTop: spacing.sm }}>
                        <View style={styles.overallRow}>
                            <ProgressRing progress={overallProgress} size={64} strokeWidth={5} color={brand.secondary}>
                                <Text style={[typography.label, { color: brand.secondary }]}>{Math.round(overallProgress * 100)}%</Text>
                            </ProgressRing>
                            <View style={{ marginLeft: spacing.md, flex: 1 }}>
                                <Text style={[typography.label, { color: colors.text }]}>{child.totalLessonsCompleted} of {totalLessons} lessons</Text>
                                <Text style={[typography.bodySmall, { color: colors.textSecondary, marginTop: 4 }]}>
                                    {overallProgress >= 1 ? "All done!" : overallProgress >= 0.5 ? "More than halfway!" : "Keep learning — every lesson counts."}
                                </Text>
                                <View style={[styles.milestoneRow, { marginTop: spacing.sm }]}>
                                    {[0.25, 0.5, 0.75, 1].map((m) => (
                                        <View key={m} style={styles.milestoneItem}>
                                            <View style={[styles.milestoneDot, { backgroundColor: overallProgress >= m ? brand.secondary : colors.surfaceTertiary, borderRadius: radius.full }]}>
                                                {overallProgress >= m && <Ionicons name="checkmark" size={8} color="#FFF" />}
                                            </View>
                                            <Text style={[typography.caption, { color: colors.textTertiary, marginTop: 2, fontSize: 10 }]}>{Math.round(m * 100)}%</Text>
                                        </View>
                                    ))}
                                </View>
                            </View>
                ...
                <Animated.View entering={FadeInDown.delay(delay(3)).duration(400)} style={{ paddingHorizontal: spacing.lg, marginTop: spacing.xxl }}>
                ...
                <Animated.View entering={FadeInDown.delay(delay(4)).duration(400)} style={{ marginTop: spacing.xxl }}>
                    <View style={[styles.achTitleRow, { paddingHorizontal: spacing.lg }]}>
                        ...
                        <View style={[styles.achCountBadge, { backgroundColor: brand.lavender + '15', borderRadius: radius.full }]}>
                            <Text style={[typography.captionBold, { color: brand.lavender }]}>{unlockedCount}/{ACHIEVEMENTS.length}</Text>
                        </View>
                    </View>
                    <View style={[styles.achProgressTrack, { marginHorizontal: spacing.lg, marginTop: spacing.sm, backgroundColor: colors.surfaceTertiary, borderRadius: radius.full, height: 4, overflow: 'hidden' }]}>
                        <View style={[styles.achProgressFill, { backgroundColor: brand.lavender, borderRadius: radius.full, height: '100%', width: `${ACHIEVEMENTS.length > 0 ? (unlockedCount / ACHIEVEMENTS.length) * 100 : 0}%` as `${number}%` }]} />
                    </View>
                    <ScrollView
                        horizontal
                        showsHorizontalScrollIndicator={false}
                        contentContainerStyle={{ paddingHorizontal: spacing.lg, paddingTop: spacing.sm, paddingBottom: spacing.lg, gap: 8 }}
                    >
                            ...
                            return (
                                <View
                                    ...
                                    style={[
                                        styles.achCard,
                                        {
                                            backgroundColor: isUnlocked ? RARITY_BG[ach.rarity] : (isDark ? colors.surfaceSecondary : colors.surface),
                                ...
                                >
                                    <View style={[styles.achIconCircle, { backgroundColor: isUnlocked ? rarityColor + '18' : colors.surfaceTertiary }]}>
                                        <Ionicons name={ACHIEVEMENT_ICONS[ach.id] ?? 'ribbon-outline'} size={16} color={isUnlocked ? rarityColor : colors.textTertiary} />
                                    </View>
                                    <Text style={[typography.captionBold, { color: isUnlocked ? colors.text : colors.textTertiary, marginTop: 6, fontSize: 11, textAlign: 'center' }]} numberOfLines={2}>
                                        {ach.title}
                                    </Text>
                                    {isUnlocked ? (
                                        <View style={[styles.rarityPill, { backgroundColor: rarityColor + '18', borderRadius: radius.full, marginTop: 5, alignSelf: 'center' }]}>
                                            <Text style={{ color: rarityColor, fontSize: 8, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.3 }}>{RARITY_LABELS[ach.rarity]}</Text>
                                        </View>
                                    ) : (
                                        <Ionicons name="lock-closed-outline" size={10} color={colors.textTertiary} style={{ marginTop: 5 }} />
                                    )}
                                </View>
                            );
                        })}
...
const styles = StyleSheet.create({
    ...
    levelBadge: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, paddingVertical: 4 },
    xpBar: { width: '100%', height: 5, overflow: 'hidden' },
    xpFill: { height: '100%' },
    statsRow: { flexDirection: 'row', justifyContent: 'space-between' },
    statBadge: { alignItems: 'center', minWidth: 56 },
    statCircle: { width: 40, height: 40, borderRadius: 20, alignItems: 'center', justifyContent: 'center' },
    ...
    achIconCircle: { width: 36, height: 36, borderRadius: 18, alignItems: 'center', justifyContent: 'center' },
