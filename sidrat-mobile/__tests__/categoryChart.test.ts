...
describe('getCompletedByCategory logic', () => {
  ...
  it('correctly counts completed lessons by category', () => {
    ...
    const result = computeCompletedByCategory(
      'child-1',
      progress,
      sampleLessons,
  ...
  it('ignores incomplete lessons', () => {
    const wuduLesson = sampleLessons.find((l) => l.category === 'wudu')!;
    const progress = [{ childId: 'child-1', lessonId: wuduLesson.id, isCompleted: false }];
  ...
  it('ignores other children progress', () => {
    const wuduLesson = sampleLessons.find((l) => l.category === 'wudu')!;
    const progress = [{ childId: 'child-OTHER', lessonId: wuduLesson.id, isCompleted: true }];
  ...
  it('counts curriculum lessons correctly', () => {
    ...
    const progress = [{ childId: 'child-1', lessonId: quranCurrLesson.id, isCompleted: true }];
