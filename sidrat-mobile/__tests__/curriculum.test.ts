...
describe('Practice Blocks', () => {
    const allPracticeBlocks: PracticeBlock[] = allCurriculumLessons.flatMap(
        (l) => l.practice,
    );
    ...
    it('ordering blocks should have at least 2 items', () => {
        const orderingBlocks = allPracticeBlocks.filter(
            (b) => b.type === 'ordering',
        );
    ...
    it('matching blocks should have matching pairs', () => {
        const matchingBlocks = allPracticeBlocks.filter(
            (b) => b.type === 'matching',
        );
